...
const PERFORMANCE_PLAN_STATE_FILE = process.env.PERFORMANCE_PLAN_STATE_FILE || './data/performance-plans.json';
const BREAK_REQUEST_CHANNEL_ID = process.env.BREAK_REQUEST_CHANNEL_ID || '1511198192211988581';
const ON_BREAK_ROLE_ID = process.env.ON_BREAK_ROLE_ID || null;
...
function validateRolePolicy() {
  ...
  const checkModmailCommand = (commandPolicy, location) => {
    ...
    const hasBuiltInRolePolicyFallback = commandPolicy.rolePolicyCommand === 'tstart';
    if (commandPolicy.rolePolicyCommand && !ROLE_POLICY.commands?.[commandPolicy.rolePolicyCommand] && !hasBuiltInRolePolicyFallback) {
...
function ticketStartRoleIds() {
  return commandRoleIds('tstart').length
    ? commandRoleIds('tstart')
    : [...new Set([
...
function performancePlanRoleIds() {
  return commandRoleIds('performancePlans').length
    ? commandRoleIds('performancePlans')
    : [...new Set([
      ...staffRoleHierarchyIds('hr'),
      ...staffRoleHierarchyIds('internals')
    ])];
...
async function loadPerformancePlanState() {
  if (!existsSync(PERFORMANCE_PLAN_STATE_FILE)) {
    await savePerformancePlanState();
    return;
  }

  const raw = await fs.readFile(PERFORMANCE_PLAN_STATE_FILE, 'utf8');
  performancePlanState = {
    plansByUserId: {},
    ...JSON.parse(raw)
  };
}

async function savePerformancePlanState() {
  await fs.mkdir('./data', { recursive: true });
  await fs.writeFile(PERFORMANCE_PLAN_STATE_FILE, `${JSON.stringify(performancePlanState, null, 2)}\n`);
}

function performancePlanStatusLabel(status) {
  return {
    active: 'Active',
    passed: 'Passed',
    failed: 'Failed',
...
function performancePlanEmbed(plan, user = null) {
...
async function memberCanManagePerformancePlans(interaction) {
  if (!interaction.guildId) return false;
  return memberHasAnyRole(interaction.guildId, interaction.user.id, performancePlanRoleIds());
}

async function performancePlanTargetMember(interaction, user) {
...
async function endStaffBreak(userId, reason) {
  ...
  if (ON_BREAK_ROLE_ID) {
    await member.roles.remove(ON_BREAK_ROLE_ID, `Staff break ended: ${reason}`).catch(() => null);
  }
...
async function restoreStaffBreaks() {
  for (const userId of Object.keys(breakState.activeByUserId)) {
    ...
    if (ON_BREAK_ROLE_ID) {
      ...
      if (!guild) continue;
      const member = await guild.members.fetch(userId).catch(() => null);
      if (member && !member.roles.cache.has(ON_BREAK_ROLE_ID)) {
        await member.roles.add(ON_BREAK_ROLE_ID, 'Staff break role restored on restart').catch(() => null);
      }
    }
  }
}
...
function applicationResponseKey(formResponse) {
...
async function checkForNewApplications({ markExistingOnFirstRun = false } = {}) {
  ...
  const processedResponseIds = new Set(reviewerState.processedResponseIds);
  ...
  if (
    ...
    reviewerState.processedResponseIds = responses.map((response) => applicationResponseKey(response));
  ...
  for (const response of newResponses) {
    ...
    reviewerState.processedResponseIds.push(responseKey);
...
async function runStartupDiagnostics({ guild, commandsRegistered }) {
  ...
  if (commandsRegistered) {
    logDiagnosticPass('slash commands /ping, /stafflist, /close, /fclose, /transfer, /refresh, /approve, /reject, /demote, /promote, /makelead, /strike, /strikes, /removestrike, /pban, /addproof, /cban, /break, /endbreak, and /listbreak');
  } else {
    logDiagnosticFail('slash commands /ping, /stafflist, /close, /fclose, /transfer, /refresh, /approve, /reject, /demote, /promote, /makelead, /strike, /strikes, /removestrike, /pban, /addproof, /cban, /break, /endbreak, and /listbreak', 'registration failed');
  }
...
async function handleDemoteCommand(interaction) {
  ...
  const inferredWing = wing ? { wingId: normalizeStaffWingId(wing), ambiguous: false } : inferredStaffWingId(targetMember);
  if (inferredWing.ambiguous) {
    await interaction.editReply(`${targetUser.username} has roles in multiple staff wings. Use the optional wing field to choose which wing to demote from.`);
    return;
  }
  ...
  if (!numberedParts.length) {
    await interaction.editReply(`The demote command is missing numbered staffHierarchy roles for ${wingName} in ${ROLE_POLICY_FILE}.`);
    return;
  }
  ...
  if (!targetPart) {
    ...
    if (assistantRankOnePart) {
      ...
      const auditReason = `Demoted from ${wingName} to Assistants by ${interaction.user.tag}: ${reason}`;
      const currentWingRoleIds = memberHierarchyPartsForWing(targetMember, wingId).map((entry) => entry.roleId);
      const assistantRoleIdsToRemove = memberHierarchyPartsForWing(targetMember, 'assistants')
        .map((entry) => entry.roleId)
        .filter((roleId) => roleId !== assistantRankOnePart.roleId);
      await targetMember.roles.add(assistantRole, auditReason);
      await targetMember.roles.remove([...new Set([
        ...currentWingRoleIds,
        ...assistantRoleIdsToRemove
      ])].filter((roleId) => roleId !== assistantRankOnePart.roleId), auditReason);
      await syncCategoryRoleForWing(targetMember, 'assistants', auditReason);
      ...
      await interaction.editReply(`${targetUser.username} has been demoted to the Assistants wing.`);
      return;
    }
    ...
    await interaction.editReply(wingId === 'assistants'
...
async function handleBreakDecision(interaction, action, userId) {
  ...
  if (ON_BREAK_ROLE_ID) {
    await member.roles.add(ON_BREAK_ROLE_ID, `Staff break approved by ${interaction.user.tag}`).catch(() => null);
  }
...
async function handleListBreakCommand(interaction) {
  if (!interaction.guildId) {
    await interaction.reply({ content: 'Command failed.', ephemeral: true });
    return;
  }

  const allowed = await memberCanUsePolicyCommand(interaction.guildId, interaction.user.id, 'manageBreaks');
  if (!allowed) {
    await interaction.reply({ content: 'Command failed.', ephemeral: true });
    return;
  }
  ...
  const activeBreaks = Object.entries(breakState.activeByUserId)
    .filter(([, activeBreak]) => activeBreak.guildId === interaction.guildId);

  if (!activeBreaks.length) {
    await interaction.editReply({
      embeds: [styledEmbed('Staff Breaks', 'No staff members are currently on break.')]
    });
    return;
  }
  ...
  await interaction.editReply({
    embeds: [styledEmbed('Staff Breaks', trimTo(lines.join('\n'), 3900))],
    allowedMentions: { parse: [] }
  });
}

async function handlePerformancePlanCommand(interaction) {
  ...
  if (subcommand === 'start') {
    ...
    if (!durationMs) {
      await interaction.reply({
        content: 'Use a due duration like `3d`, `1 week`, or `12h`.',
        ephemeral: true
      });
      return;
    }
    ...
    if (!member) {
      await interaction.editReply('That user is not a configured staff member.');
      return;
    }
    const wing = interaction.options.getString('wing');
    const inferredWing = wing ? { wingId: normalizeStaffWingId(wing), ambiguous: false } : inferredStaffWingId(member);
    if (inferredWing.ambiguous) {
      await interaction.editReply(`${targetUser.tag} has roles in multiple staff wings. Use the optional wing field to choose which wing this plan applies to.`);
      return;
    }
    const wingId = inferredWing.wingId;
    if (!wingId) {
      await interaction.editReply('I could not detect that staff member\'s wing.');
      return;
    }

    const existingPlan = performancePlanState.plansByUserId[targetUser.id];
    if (existingPlan?.status === 'active') {
      await interaction.editReply(`${targetUser.tag} already has an active performance plan.`);
      return;
    }
    ...
    performancePlanState.plansByUserId[targetUser.id] = plan;
    await savePerformancePlanState();

    await targetUser.send({
      embeds: [
        styledEmbed(
          'Performance Plan Started',
          [
    ...
    await interaction.editReply({
      content: `Started a performance plan for ${targetUser.tag}.`,
      embeds: [performancePlanEmbed(plan, targetUser)],
      allowedMentions: { parse: [] }
    });
    return;
  }
  ...
  if (subcommand === 'view') {
    ...
    if (!member) {
      await interaction.reply({
        content: 'That user is not a configured staff member.',
        ephemeral: true
      });
      return;
    }

    const plan = performancePlanState.plansByUserId[targetUser.id];
    if (!plan) {
      await interaction.reply({
        content: `${targetUser.tag} does not have a performance plan on record.`,
        ephemeral: true
      });
      return;
    }
  ...
  if (subcommand === 'note') {
    ...
    if (!plan || plan.status !== 'active') {
      await interaction.reply({
        content: `${targetUser.tag} does not have an active performance plan.`,
        ephemeral: true
      });
      return;
    }
    ...
    await interaction.reply({
      content: `Added a note to ${targetUser.tag}'s performance plan.`,
      ephemeral: true
    });
    return;
  }
  ...
  if (subcommand === 'complete') {
    ...
    await interaction.reply({
      content: `Marked ${targetUser.tag}'s performance plan as ${performancePlanStatusLabel(result)}.`,
      embeds: [performancePlanEmbed(plan, targetUser)],
      ephemeral: true,
      allowedMentions: { parse: [] }
    });
  ...
  if (subcommand === 'list') {
    const wingId = normalizeStaffWingId(interaction.options.getString('wing'));
    const plans = Object.values(performancePlanState.plansByUserId)
...
async function handleStaffListCommand(interaction) {
  ...
  const allBreakUserIds = Object.entries(breakState.activeByUserId)
    .filter(([, activeBreak]) => activeBreak.guildId === interaction.guildId)
    .map(([userId]) => userId);
...
async function handleTicketStartCommand(interaction) {
  const allowed = interaction.guildId
    ? await memberHasAnyRole(interaction.guildId, interaction.user.id, ticketStartRoleIds())
    : false;
  if (!allowed) {
    await interaction.reply({
      content: 'You do not have permission to use this command.',
  ...
  const targetUser = interaction.options.getUser('user', true);
  const wing = interaction.options.getString('wing', true);
  const department = departmentFor(wing);
  if (!department) {
    await interaction.reply({
      content: 'That ticket wing is not configured.',
      ephemeral: true
    });
    return;
  }
  ...
  const existing = await activeChannelFor(targetUser.id, interaction.guildId);
  if (existing || state.pendingByUserId[targetUser.id]) {
    await interaction.editReply(`${targetUser.tag} already has an open or pending ticket.`);
    return;
  }

  const channel = await openTicket(targetUser, department.id, interaction.guildId);
  if (!channel) {
    await interaction.editReply(`I could not open a ${department.label} ticket for ${targetUser.tag}.`);
    return;
  }
  ...
  await targetUser.send({
    embeds: [
      styledEmbed(
        'Ticket Opened',
        [
          `A staff member opened a ticket for you with the **${teamNameForDepartment(department.id)}**.`,
          '',
  ...
  await channel.send({
    embeds: [
      styledEmbed(
        'Ticket Opened By Staff',
        `Opened by **${interaction.user.tag}** for **${targetUser.tag}**.`
      )
    ],
    allowedMentions: { parse: [] }
  }).catch(() => null);

  await interaction.editReply(`Opened a ${department.label} ticket for ${targetUser.tag}: <#${channel.id}>`);
}
  ...
  await guild.commands.set([
    ...
    new SlashCommandBuilder()
      ...
      .addStringOption((option) =>
        option
          .setName('wing')
          .setDescription('Optional wing to demote from if auto-detection is ambiguous.')
          .setRequired(false)
    ...
    new SlashCommandBuilder()
      ...
      .addSubcommand((subcommand) =>
        subcommand
          .setName('start')
          .setDescription('Start a performance plan for a staff member.')
          .addUserOption((option) =>
            option
              .setName('user')
              .setDescription('The staff member to place on a plan.')
              .setRequired(true)
          )
          .addStringOption((option) =>
            option
              .setName('due')
              .setDescription('When this plan is due, such as 3d or 1 week.')
              .setMaxLength(100)
              .setRequired(true)
          )
          .addStringOption((option) =>
            option
              .setName('reason')
              .setDescription('Why this plan is being started.')
              .setMaxLength(1000)
              .setRequired(true)
          )
          .addStringOption((option) =>
            option
              .setName('goals')
              .setDescription('What the staff member needs to improve or complete.')
              .setMaxLength(1000)
              .setRequired(true)
          )
          .addStringOption((option) =>
            option
              .setName('wing')
              .setDescription('Optional wing if auto-detection is ambiguous.')
              .setRequired(false)
              .addChoices(...WING_COMMAND_CHOICES)
          )
      )
      .addSubcommand((subcommand) =>
        subcommand
          .setName('view')
          .setDescription('View a staff member performance plan.')
          .addUserOption((option) =>
            option
              .setName('user')
      ...
      .addSubcommand((subcommand) =>
        subcommand
          .setName('note')
          .setDescription('Add a note to an active performance plan.')
          .addUserOption((option) =>
            option
              .setName('user')
              .setDescription('The staff member to note.')
              .setRequired(true)
          )
          .addStringOption((option) =>
            option
      ...
      .addSubcommand((subcommand) =>
        subcommand
          .setName('complete')
          .setDescription('Complete an active performance plan.')
          .addUserOption((option) =>
            option
              .setName('user')
              .setDescription('The staff member to complete.')
              .setRequired(true)
          )
          .addStringOption((option) =>
            option
              .setName('result')
              .setDescription('How this plan ended.')
              .setRequired(true)
              .addChoices(
          ...
          .addStringOption((option) =>
            option
              .setName('note')
              .setDescription('Result note to save and DM.')
              .setMaxLength(1000)
              .setRequired(true)
          )
      )
      .addSubcommand((subcommand) =>
        subcommand
          .setName('list')
          .setDescription('List active performance plans.')
          .addStringOption((option) =>
            option
              .setName('wing')
              .setDescription('Only list plans for this wing.')
              .setRequired(false)
              .addChoices(...WING_COMMAND_CHOICES)
          )
      )
    ...
    new SlashCommandBuilder()
      .setName('tstart')
      .setDescription('Open a modmail ticket for a user.')
      .addUserOption((option) =>
        option
          .setName('user')
      ...
      .addStringOption((option) =>
        option
          .setName('wing')
          .setDescription('The wing to open this ticket under.')
          .setRequired(true)
          .addChoices(...WING_COMMAND_CHOICES)
      )
      .toJSON(),
...
client.once('ready', async () => {
  ...
  await loadPerformancePlanState();
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.commandName === 'listbreak') {
      await handleListBreakCommand(interaction);
      return;
    }

    if (interaction.commandName === 'pplan') {
      await handlePerformancePlanCommand(interaction);
      return;
    }
    ...
    if (interaction.commandName === 'tstart') {
      await handleTicketStartCommand(interaction);
      return;
    }
